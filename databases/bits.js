...
class BitsQueries {
	constructor(params) {
		...
		this.db = new BitsDBAdapter({ cryptr: this.cryptr, dataDir: this.dataDir }).get();
	}
	...
	addUser(user, amount) {
		...
		return new Promise(function (resolve, reject) {
			if (db.get(constants.DATABASE_NAMES.BITS).find({ name: user }).has("name").value()) {
				...
				this.db.get(constants.DATABASE_NAMES.BITS).find({ name: user }).assign({ name: user, amount: newAmount }).write();
			} else {
				this.db.get(constants.DATABASE_NAMES.BITS).push({ name: user, amount: amount }).write();
			}
